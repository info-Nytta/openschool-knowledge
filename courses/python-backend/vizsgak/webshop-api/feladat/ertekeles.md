...
| Szempont | Pont |
|----------|------|
| `TermekCreate` séma – mezők és típusok helyesek | 1 |
| `TermekCreate` – validáció (min_length, ge) | 1 |
| `TermekRead` séma – id, aktiv, letrehozva mezők | 1 |
| `TermekUpdate` séma – opcionális mezők | 1 |
...
| Szempont | Pont |
|----------|------|
| `ar_formatum()` – helyes formázás ezres elválasztóval | 1 |
| `keszleten_van()` – helyes ellenőrzés | 1 |
...
| `models.py` – `TermekModel` definiálva, mezők helyesek | 2 |
...
| `GET /termekek` – listázás SQLAlchemy query-vel | 2 |
| `GET /termekek` – query paraméter szűrés (kategoria, min_ar, max_ar) | 2 |
| `GET /termekek/{termek_id}` – létező termék visszaadása | 1 |
| `GET /termekek/{termek_id}` – 404 hiba kezelés | 1 |
| `POST /termekek` – létrehozás adatbázisba (201 státusz) | 2 |
| `PUT /termekek/{termek_id}` – módosítás (exclude_unset) | 1 |
| `DELETE /termekek/{termek_id}` – törlés + 404 kezelés | 1 |
| `GET /termekek/statisztika` – helyes eredmény | 1 |
| `termekek.txt` betöltése induláskor (ha üres a tábla) | 1 |
...
| `atlag_ar()` – helyes átlagár, kerekítés | 2 |
| `legdragabb()` – helyes keresés | 2 |
...
| Szempont | Pont |
|----------|------|
| `test_root` + `test_get_termekek` – alap végpontok tesztelése | 1 |
| `test_create_termek_unauthorized` + `test_delete_unauthorized` – 401 tesztelés | 1 |
| `test_register` + `test_login` – auth végpontok tesztelése | 1 |
| `test_create_termek_with_auth` – auth flow tesztelése | 1 |
