...
2. DNS rekord: `openschool.hu` → VPS IP (A rekord)
3. SSL mód: Full (strict)
4. Cloudflare automatikusan kezeli az SSL tanúsítványt

**Staging domain:**
- `staging.openschool.hu` → ugyanaz a VPS, külön Docker Compose konfig

**Feladat:**
- Állítsd be a DNS rekordokat
- Cloudflare SSL: Full (strict)
- Teszteld: `https://openschool.hu` elérhető (egyelőre akár egy statikus oldallal)
...
|-----------|--------|--------|-----|
| Staging | `staging.openschool.hu` | `develop` | Tesztelés éles-szerű környezetben |
| Production | `openschool.hu` | `main` | Éles, felhasználók által használt |
...
```
0 3 * * * /opt/openschool/scripts/backup.sh
```
...
- [ ] A platform fut a VPS-en: `https://openschool.hu` elérhető
