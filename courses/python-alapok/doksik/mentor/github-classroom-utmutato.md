# GitHub Classroom – Mentori útmutató a házi feladatokhoz
...
### 1.3 Tanulók meghívása

1. A Classroom oldalán kattints a **Students** fülre
2. Itt két lehetőséged van:
   - **Meghívó link** küldése (a tanulók magukat adják hozzá)
   - **Névsor feltöltése** (CSV fájl, hogy a tanulók nevüket kiválaszthassák)
...
- Küldd el a meghívó linket a tanulóknak (Teams, email, Kréta üzenet, stb.)
- A tanuló rákattint → elfogadja → automatikusan létrejön a saját privát repoja
...
Minden template repo tartalmaz automatikus teszteket (`.github/classroom/autograding.json`), amelyek **automatikusan lefutnak**, amikor a tanuló pushol. Nem kell ehhez semmit beállítanod — a tesztek a template repo részeként kerülnek a tanulók repójába.

### 4.1 Hogyan működik?

1. A tanuló pushol → a GitHub Actions automatikusan lefuttatja a teszteket
2. A Classroom felületén megjelenik az eredmény (pl. „7/10 pont")
3. A tanuló is látja a saját eredményét a repojában (Actions fül → zöld ✅ vagy piros ❌)
...
|-------------|-------|---------------|
| **Fájl létezik** | `test -f feladat1.py` | A tanuló létrehozta-e a fájlt |
...
- A tesztek **nem tökéletesek** — nem helyettesítik a mentori ellenőrzést, de jó első szűrőt adnak
- Ha egy tanuló „átmegy" a teszteken, az nem jelenti, hogy a kód szép vagy jól strukturált
- A tesztek inkább **ösztönzőként** működnek: a tanuló azonnal látja, hogy helyes-e a megoldása
...
2. Látod az összes tanuló státuszát:
...
- Kattints egy tanuló repojára → a GitHub oldalon megnyílik
...
- **Névsor feltöltése** a Classroom-ba: így a tanulók nevéhez rendelheted a GitHub felhasználónevüket
- **Privát repok** használata: a tanulók nem látják egymás megoldásait
- **GitHub Classroom Assistant** alkalmazás: egyszerre le tudod tölteni az összes tanuló repóját → [classroom.github.com/assistant](https://classroom.github.com/assistant)
- **Nem kell minden héten minden feladatot részletesen ellenőrizni** — a fontos az, hogy a tanuló rendszeresen gyakoroljon és commitoljon
